--select * from users
--select * from Policy_Payout
--select * from policy

--select p.CreatedBy,* from Policy p inner join Policy_Payout po on p.id = po.PolicyId
 exec GetPolicyAggregationsByUserReporties 1
...
BEGIN
	...
	, (select count(id) from ##emp) as NoOfReports
