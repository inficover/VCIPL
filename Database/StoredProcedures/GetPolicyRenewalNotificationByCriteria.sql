CREATE PROCEDURE [dbo].[GetPolicyRenewalNotificationByCriteria]
	@UserId int,
	--@DirectReportId int, 
...
BEGIN
	...
	select  p.RegisteredDate, p.PolicyNumber, p.id, p.RegistrationNo, p.GrossPremium, p.NetPremium, p.ODPremium, prn.Status as StatusId,p.CPS, p.RSD, p.RED, p.PolicyIssuenceDate,pmodes.Name as PaymentMode, p.PaymentModeOthers,
	...
	left join PolicyRenewalNotificationStatus prns on prns.Id = prn.Status
